...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    compute_dist_loss = ComputeDstillLoss(model, tenmperture=opt.temperature)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.distill: mdloss = torch.zeros(1, device=device)
        if rank != -1:
            dataloader.sampler.set_epoch(epoch)
        pbar = enumerate(dataloader)
        if opt.distill:
            logger.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'distill', 'targets', 'img_size'))
        else:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                mdloss = (mdloss * i + dloss) / (i + 1)  # update mean dloss
                mem = '%.3gG' % (torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                s = ('%10s' * 2 + '%10.4g' * 6+ '%10.4g') % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, mdloss, targets.shape[0], imgs.shape[-1])
        ...
        if rank in [-1, 0]:
            ...
            if fi > best_fitness:
                best_fitness = fi
            wandb.end_epoch(best_result=best_fitness == fi)
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                if wandb.wandb:
                    if ((epoch + 1) % opt.save_period == 0 and not final_epoch) and opt.save_period != -1:
                        wandb.log_model(last.parent, opt, epoch, fi, best_model=best_fitness == fi)
                del ckpt
    ...
    if rank in [-1, 0]:
        ...
        if wandb.wandb and not opt.evolve:  # Log the stripped model
            wandb.wandb.log_artifact(str(final), type='model',
                                            name='run_' + wandb.wandb_run.id + '_model',
