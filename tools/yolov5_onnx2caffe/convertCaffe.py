...
def convertToCaffe(graph, prototxt_save_path, caffe_model_save_path, exis_focus=False, focus_concat_name=None,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--onnx-path', '--path', type=str, default='')
    parser.add_argument('--onnx-name', '--name', type=str, default='')
    opt = parser.parse_args()

    onnx_name = opt.name + '-simple.onnx'
    onnx_path = os.path.join(opt.path, onnx_name)
    prototxt_path = os.path.join(opt.path, onnx_name.split(".")[0]) + '.prototxt'
    hisiprototxt_path = os.path.join(opt.path, onnx_name.split(".")[0]) + '-hisi.prototxt'
    caffemodel_path = os.path.join(opt.path, onnx_name.split(".")[0]) + '.caffemodel'

    graph = getGraph(onnx_path)

    convertToCaffe(graph, prototxt_path, caffemodel_path, exis_focus=True, focus_concat_name="Concat_40",
                   focus_conv_name="Conv_41")
