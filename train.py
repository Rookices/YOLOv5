...
from utils.metrics import calculate_f1, fitness
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    elif pretrained and opt.ft_pruned:  # Fine-tuning pruned model
        print("[FT] Restoring pruned model", weights)
        ckpt = torch.load(weights, map_location=device)  # load checkpoint
        model = ckpt["model"]
        model.info()
        csd = {}
    ...
    if opt.sl_factor > 0:
        ...
        # ignore_idx = [230, 260, 290] # 可添加忽略的bn层
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        if pretrained:
            ...
            if opt.f1_factor > 0:
                ...
                best1 = wdir / best_F1
            last = wdir / last_
            if opt.remove_aug > 0 and epoch >= epochs - opt.remove_aug:
                hyp['mosaic'] = 0.0
            # sr_cos
            if opt.sl_factor > 0 and opt.sr_cos:
                opt.sl_factor *= ((((1 + math.cos(epoch * math.pi / epochs)) / 2) ** 1.0) * 0.8 + 0.2)
            best = wdir / best_mAP
        model.train()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale > 0:
                sz = random.randrange(imgsz * (1 - opt.multi_scale),
                                      imgsz * (1 + opt.multi_scale) + gs) // gs * gs  # size(±50%)
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if epoch % opt.save_period == 0 or epoch >= epochs - 1:
                    torch.save(ckpt, last)
                if best_fitness == fi:
                    torch.save(ckpt, best)
                if opt.f1_factor > 0 and best_f1 == f1:
            ...
            if is_coco:  # COCO dataset
                for m in [last, best] if best.exists() else [last]:  # speed, mAP tests
                    results, _, _ = val.run(data_dict,
                                            ...
                                            plots=False)
            # Strip optimizers
            for f in ((last, best) if not opt.f1_factor > 0 else (best, best1)):
                if f.exists():
...
def parse_opt(known=False):
    ...
    parser.add_argument('--multi-scale', type=int, default=0, help='scale the img-size, range from 0 to 1')
    ...
    parser.add_argument('--urlPath', default='./data/urlTxtPath', type=str, help='image url txt Path')
...
def main(opt):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'cutmix': (1, 0.0, 1.0)}  # image mixup (probability)
