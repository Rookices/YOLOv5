...
class Dependency(object):
    ...
    def __str__(self):
        return "<DEP: %s => %s on %s>" % (
        "None" if self.trigger is None else self.trigger.__name__, self.handler.__name__, self.broken_node.node_name)
...
class DependencyGraph(object):
    ...
    def _obtain_forward_graph(self, model, example_inputs, output_transform):
        ...
        hooks = [m.register_forward_hook(_record_module_grad_fn) for m in model.modules() if
                 isinstance(m, tuple(self.PRUNABLE_MODULES))]
        ...
        def _build_graph(grad_fn):
            ...
            if module is None:
                if not hasattr(grad_fn, 'name'):
                    ...
                    if self.verbose:
                        print("[Warning] Unrecognized operation: %s. It will be treated as element-wise op" % (
                            str(grad_fn)))
